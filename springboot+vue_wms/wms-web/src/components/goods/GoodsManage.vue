<template>
    <div>
        <div style="margin-bottom: 5px;">
            ...
            <el-button type="danger" style="float: right; margin-right: 5px;" @click="outGoods" v-if="user.roleId!=2">出库</el-button>
            <el-button type="primary" style="float: right" @click="inGoods" v-if="user.roleId!=2">入库</el-button>
        </div>
...
<!--            </el-table-column>-->
            ...
            <el-table-column prop="operate" label="操作" fixed="right" width="170" v-if="user.roleId!=2">
        ...
        methods:{
            ...
            loadPost(){
                this.$axios.post(this.$httpUrl+'/goods/page',{
                    ...
                    param:{
                        name:this.name,
                        storage:this.storage+'', // +''是为了确保是字符型
            ...
            doMod(){
                this.$axios.put(this.$httpUrl+'/goods/update',this.form).then(res=>res.data).then(res=>{
                    // console.log(res)
                    if(res.code==200){
                        this.$message({
                            message: '操作成功！',
            ...
            del(id){
                // console.log(id)
                this.$axios.delete(this.$httpUrl+'/goods/del?id='+id).then(res=>res.data).then(res=>{
                    // console.log(res)
                    if(res.code==200){
                        this.$message({
                            message: '操作成功！',
            ...
            formatStorage(row){
                // let obj = this.storageData ?.find(item => {
